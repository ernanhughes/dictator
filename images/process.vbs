...
Function process2(file, width, height, directory)
	...
	If Not fso.FolderExists(directoryPath) Then
		Wscript.Echo("Creating directory: " & directoryPath)
		Call fso.CreateFolder(directoryPath)
	End If
	sNewFileName = directoryPath & "\" & Replace(file.Name, ".svg", ".png")
	If fso.FileExists(sNewFileName) Then
		Call fso.DeleteFile(sNewFileName)
	End If
	strCommand = """C:\Program Files\inkscape\inkscape.exe"" " & file.Path & " -z -e -w " & width & " -h " & height & " --export-dpi=1200 --export-area-drawing --export-png=" & sNewFileName
    Wscript.Echo(strCommand)
	shell.run strCommand, 0, False
	strCommand = "mogrify   " & sNewFileName & " -background none -gravity Center -extent  " & width & "x" & height & " " & sNewFileName
    Wscript.Echo(strCommand)
	shell.run strCommand, 0, True
End Function
...
Function getMenuIconSizes(file)
	Dim list: Set list = CreateObject("Scripting.Dictionary")
	list.Add "96", "mipmap-xxhdpi"
	list.Add "72", "mipmap-xxhdpi"
	list.Add "48", "mipmap-xhdpi"
	list.Add "36", "mipmap-hdpi"
	list.Add "24", "mipmap-mdpi"
